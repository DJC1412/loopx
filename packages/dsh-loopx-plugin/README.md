...
- the package-root Host registers the GoalBar carrier on the running
  generation's authenticated Connection channel — the shared
  `/api/loopx.goalbar` route where the host exposes a Fetch face, and the
  standalone `/loopx` channel otherwise — and its web Client contributes a
  compact GoalBar between DSH's native GoalBar and Queue dock rows. It renders
  only for one exact live `(goalId, loopxAgentId)` binding.
...
The GoalBar carrier is registered through Connection's own authenticated
registry: the shared `/api/loopx.goalbar` route where the installed generation
exposes a Fetch face, and the standalone `/loopx` channel otherwise. Both pass
