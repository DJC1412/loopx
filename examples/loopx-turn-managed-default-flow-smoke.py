#!/usr/bin/env python3
"""Qualify the operator default flow for one bounded managed Turn.

The shipped operator rule is credential-resolved: a configured operator model
credential selects the managed ``dsh`` executor, and no credential keeps the
individual Codex CLI default. That rule is only usable if the *default* command
(no explicit ``--host``) actually starts the managed Turn and reports what ran.
...
1. no credential: the default host is ``codex-cli`` with an individual executor;
2. credential: the default host is ``dsh`` with a managed executor that reports
   its credential environment and launchability before any work runs;
...
def _plan_argv(registry: Path, runtime: Path, project: Path) -> list[str]:
    return [
        "--registry",
        str(registry),
        "--runtime-root",
...
def _run_once_argv(
    *,
    ...
    # No --host: the default must come from the operator credential, which is
    # the surface this smoke qualifies.
    argv = [
        "--registry",
        str(registry),
        "--runtime-root",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="loopx-managed-default-flow-") as directory:
        ...
        ambient = {
            ...
            for key in (ENDPOINT_ENV, CREDENTIAL_ENV, "DSH_CWD", "DSH_HOME", "DSH_SESSION_ROOT")
        }
        try:
            ...
            summary = {
                ...
                "managed_default_plan": {
                    ...
                    "execution_mode": managed_plan.get("host", {}).get("execution_mode"),
                    "executor": managed.get("executor"),
                    "executor_kind": managed.get("executor_kind"),
                    "credential_env": managed.get("credential_env"),
                    "operator_credential_bound": managed.get("operator_credential_bound"),
                ...
                "managed_default_run": {
                    ...
                    "validation_status": (run_payload.get("validation") or {}).get("status"),
                ...
                "managed_runtime_unavailable": {
                    ...
                    "executor_kind": unavailable_payload.get("managed_executor", {}).get(
                        "executor_kind"
                    ),
                },
    ...
    ok = (
        individual_exit == 0
        and summary["individual_default"]["host_kind"] == "codex-cli"
        and summary["individual_default"]["executor_kind"] == EXECUTOR_KIND_INDIVIDUAL
